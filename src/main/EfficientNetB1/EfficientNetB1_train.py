...
valid_generator = generator.flow_from_directory(
    ...
    seed=6303)
#%%
base_model = FineTuneModel(model_name='EfficientNetB1', input_shape=(224, 224, 3), num_classes=2)
...
plot_loss(history=base_history, model_name='EfficientNetB1_ela', layers_info='Pretrained Layers Frozen', image_name='EfficientNetB1_pretrained_ela')

#%%
base_model.unfreeze_layers(num_layers=80)
...
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.25,
                              patience=5)
checkpoint = ModelCheckpoint('finetuned_EfficientNetB1_ela.keras',save_best_only=True, monitor='val_loss', verbose=1, mode='min')
...
plot_loss(history=base_history_ft, model_name='EfficientNetB1_ela', layers_info='All Layers Unfrozen', image_name='EfficientNetB1_finetuned_ela')

# Save the fine-tuned model
name = 'finetuned_EfficientNetB1_ela'
